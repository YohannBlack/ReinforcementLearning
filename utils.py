...
def execute_comparison_mc(env, nb_iter, max_step, gamma):
    ...
    start = time.time()
    _, _, cummu_avg_r_es = monte_carlo_with_exploring_start(
        env, nb_iter=nb_iter, max_step=max_step, GAMMA=gamma)
    exec_time['exploring_start'] = time.time() - start

    start = time.time()
    _, _, cummu_avg_r_onp = monte_carlo_on_policy(
        env, nb_iter=nb_iter, max_step=max_step, GAMMA=gamma)
    exec_time['on_policy'] = time.time() - start

    start = time.time()
    _, _, cummu_avg_r_ofp, mean_q_ofp = monte_carlo_off_policy(
        env, nb_iter=nb_iter, max_step=max_step, gamma=gamma)
...
def execute_comparison_td(env, nb_iter, alpha, gamma, epsilon, max_step):
    ...
    return cumm_avg_sarsa, cumm_avg_q_l, exec_time
...
def visualize_temporal_difference(sarsa, q_learning, execution_time, env_name):
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(15, 6))

    ax1.plot(np.arange(len(sarsa)),
             sarsa, label='SARSA', color='blue')
    ax1.plot(np.arange(len(q_learning)), q_learning,
             label='Q-Learning', color='red')
    ax1.set_title(
        'Cumulative Reward of TD SARSA vs Q-Learning')
    ax1.set_xlabel('Episodes')
    ax1.set_ylabel('Cumulative Reward')
    ax1.legend()
    ax1.grid(True)
    ...
    ax2.set_title('Execution Time of TD SARSA vs Q-Learning')
    ax2.set_xlabel('Algorithm')
    ax2.set_ylabel('Time (seconds)')
